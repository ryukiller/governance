...
describe('GovernorAlpha', () => {
  ...
  it('ari', async () => {
    const balance = await ari.balanceOf(wallet.address)
    const totalSupply = await ari.totalSupply()
  ...
  it('governor', async () => {
    ...
    const ariFromGovernor = await governorAlpha.ari()
    expect(ariFromGovernor).to.be.eq(ari.address)
  })
})
