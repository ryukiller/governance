...
export async function governanceFixture(
  ...
  // deploy ARI, sending the total supply to the deployer
  const { timestamp: now } = await provider.getBlock('latest')
  const timelockAddress = Contract.getContractAddress({ from: wallet.address, nonce: 1 })
  const ari = await deployContract(wallet, Ari, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, ari.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { ari, timelock, governorAlpha }
}
