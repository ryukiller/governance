...
import AriswapV2Pair from '@ariswap/v2-core/build/AriswapV2Pair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy ariswap v2', async () => {
    factory = await deployContract(wallet, AriswapV2Factory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, Ari, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, Ari, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, AriswapV2Pair.abi).connect(wallet)
