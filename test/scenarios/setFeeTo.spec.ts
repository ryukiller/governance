...
describe('scenario:setFeeTo', () => {
  ...
  beforeEach('deploy ariswap v2', async () => {
    factory = await deployContract(wallet, AriswapV2Factory, [timelock.address])
  })
  ...
  it('setFeeTo', async () => {
    ...
    const description = 'Set feeTo on the AriswapV2Factory to the timelock address.'

    // activate balances
    await ari.delegate(wallet.address)
